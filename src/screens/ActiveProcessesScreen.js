...
export default function ActiveProcessesScreen() {
  ...
  async function handleLoadData(refresh = false) {
    ...
    try {
      const response = await ADVService.get(
        `/api/v1/app/processos/ativos/paginate?page=${currentPage}`,
      );
    ...
    <NajContainer style={styles.container}>
      {(!loading && (
        <ProcessList
          ...
          ListEmptyComponent={() => (
            <EmptyList text="Nenhum registro de processo ativo encontrado" />
          )}
          refreshControl={
