...
export default function ToReceiveScreen({ values }) {
  const financeFilter = useSelector(state => state.auth.financeFilter);
  const toReceiveValue = values.toReceiveValue;
  const toReceiveValueOpened = values.toReceiveValueFinished;
    ...
    return (
      <View style={[styles.listItem, styles.listHeader]}>
        <View style={styles.listHeaderCounter}>
          <NajText style={styles.listHeaderTitle}>Recebido</NajText>
          <NajText style={styles.listHeaderValue}>
        ...
        <View style={styles.listHeaderCounter}>
          ...
          <NajText
            style={[styles.listHeaderValue, { color: 'rgb(46, 159, 209)' }]}>
            {applyMoneyMask(toReceiveValueOpened)}
          </NajText>
        </View>
      </View>
    );
    ...
    return (
      <View style={[styles.listItem, { backgroundColor: itemBgColor }]}>
        ...
        <View style={styles.listRow}>
          ...
          {item.DATA_PAGAMENTO && (
            <View style={{ flex: 1 }}>
              <NajText style={styles.nonInfo}>Recebido em</NajText>
              <NajText style={styles.listDateText}>
                {item.DATA_PAGAMENTO}
  ...
  async function handleLoadData(refresh = false) {
    ...
    try {
      const response = await ADVService.get(
        `/api/v1/app/financeiro/receber/paginate?page=${currentPage}`,
      );
      ...
      {(!loading && (
        <>
          ...
          <FlatList
            ...
            ListEmptyComponent={() => (
              <EmptyList text="Nenhum registro de conta a receber encontrado" />
            )}
